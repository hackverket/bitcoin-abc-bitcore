...
static bool AcceptToMemoryPoolWorker(
    ...
    if (false && !ContextualCheckTransactionForCurrentBlock(
...

    {
        ...
        Amount mempoolRejectFee =
            pool.GetMinFee(
                    ...
                    1000000)
                .GetFee(nSize);
        if (false && mempoolRejectFee > Amount(0) && nModifiedFees < mempoolRejectFee) {
            return state.DoS(0, false, REJECT_INSUFFICIENTFEE,
                             "mempool min fee not met", false,
                             strprintf("%d < %d", nFees, mempoolRejectFee));
        }

        if (false && gArgs.GetBoolArg("-relaypriority", DEFAULT_RELAYPRIORITY) &&
        ...
        if (false && nAbsurdFee != Amount(0) && nFees > nAbsurdFee) {
...
static bool AcceptToMemoryPoolWorkerNonStandard(
    const Config &config, CTxMemPool &pool, CValidationState &state,
    const CTransactionRef &ptx, bool fLimitFree, bool *pfMissingInputs,
    int64_t nAcceptTime, bool fOverrideMempoolLimit, const Amount nAbsurdFee,
    std::vector<COutPoint> &coins_to_uncache) {
    ...
    // Rather not work on nonstandard transactions (unless -testnet/-regtest)
    std::string reason;
    if (false && fRequireStandard && !IsStandardTx(tx, reason, hasMonolith)) {
        return state.DoS(0, false, REJECT_NONSTANDARD, reason);
    }
 ...
 * (try to) add transaction to memory pool with a specified acceptance time.
 */
static bool AcceptToMemoryPoolWithTimeNonStandard(const Config &config, CTxMemPool &pool,
                                       CValidationState &state,
                                       const CTransactionRef &tx,
                                       bool fLimitFree, bool *pfMissingInputs,
                                       int64_t nAcceptTime,
                                       ...
                                       const Amount nAbsurdFee = Amount(0)) {
    std::vector<COutPoint> coins_to_uncache;
    bool res = AcceptToMemoryPoolWorkerNonStandard(
        config, pool, state, tx, fLimitFree, pfMissingInputs, nAcceptTime,
        fOverrideMempoolLimit, nAbsurdFee, coins_to_uncache);
    if (!res) {
        for (const COutPoint &outpoint : coins_to_uncache) {
...
bool AcceptToMemoryPoolNonStandard(const Config &config, CTxMemPool &pool,
                        CValidationState &state, const CTransactionRef &tx,
                        bool fLimitFree, bool *pfMissingInputs,
                        bool fOverrideMempoolLimit, const Amount nAbsurdFee) {
    return AcceptToMemoryPoolWithTimeNonStandard(config, pool, state, tx, fLimitFree,
                                      pfMissingInputs, GetTime(),
                                      fOverrideMempoolLimit, nAbsurdFee);
}
